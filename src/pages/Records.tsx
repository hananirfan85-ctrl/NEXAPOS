...
export default function Records() {
  ...
  const getIcon = (action: string) => {
    switch (action) {
      case "ADD_PRODUCT":
        return <PlusSquare size={16} className="text-emerald-600" />;
      case "UPDATE_PRODUCT":
  ...
  const clearLogs = async () => {
    if (!window.confirm("Are you sure you want to clear all activity records?"))
      return;
    setLoading(true);
    await supabase
      .from("activity_logs")
      .delete()
      .neq("id", "00000000-0000-0000-0000-000000000000"); // Deletes all user's logs (RLS protects others)
    fetchLogs();
  };
          ...
          <div className="relative border-l border-gray-200 ml-4 space-y-8 pb-4">
            {logs.map((log) => (
              <div key={log.id} className="relative pl-8">
                {/* Timeline dot */}
                <div
                  className={`absolute -left-3.5 top-1 w-7 h-7 rounded-full border-2 flex items-center justify-center ${getColor(log.action)}`}
                >
                  {getIcon(log.action)}
                </div>
