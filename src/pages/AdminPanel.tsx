...
export default function AdminPanel() {
  ...
  const [activeTab, setActiveTab] = useState<"clients" | "messages" | "backup">(
    "clients",
  );
  ...
  const fetchMessages = async () => {
    ...
    if (error) {
      if (
        error.message.includes('relation "public.user_messages" does not exist')
      ) {
        setNeedsSetup(true);
      } else {
    ...
    if (error || (roleError && roleError.message.includes("Could not find"))) {
      if (
        ...
        setError(
          error
            ? error.message
            : "Missing get_my_role function. Please run the SQL setup code again.",
        ); // store it anyway so we can see it
      } else {
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Manage platform clients, support tickets, and system health.
          </p>
        </div>
        ...
        <div className="bg-indigo-50 border border-indigo-200 text-indigo-800 p-4 rounded-xl flex gap-3 text-sm">
          ...
          <div>
            <strong className="block mb-1 text-base">
              Important Notice Regarding Clients Approvals:
            </strong>
            Since you are testing this on your deployed Vercel application
            (nexapossystem.vercel.app), your clients will{" "}
            <strong>still see the pending approval page</strong> until you
            actually{" "}
            <strong>
              push these new code updates to your GitHub repository
            </strong>{" "}
            so Vercel can rebuild the app with the newly added backend SQL
            integrations. <br className="my-2" /> Make sure clients refresh
            their page after the new deployment finishes on Vercel.
          </div>
        </div>
      )}
      ...
      {needsSetup ? (
        <div className="bg-white rounded-xl shadow-sm border border-orange-200 overflow-hidden">
          {error && (
            <div className="p-3 bg-red-50 text-red-600 text-sm font-mono border-b border-red-200">
              Error: {error}
            </div>
          )}
          <div className="p-6 bg-orange-50 border-b border-orange-200 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-orange-800 mt-1">
                To securely view all authenticated users and grant them access,
                you need to sync the new user_roles functions.
              </p>
            </div>
          </div>
          <div className="p-6 space-y-4">
            ...
            <div className="space-y-2 mt-6">
              ...
              <ul className="list-decimal pl-5 text-gray-600 space-y-1">
                ...
                <li>
                  Click on <strong>SQL Editor</strong> on the left sidebar
                </li>
                <li>
                  Pase the code and click <strong>Run</strong>
                </li>
              </ul>
            </div>
            <div className="pt-4 border-t border-gray-100 mt-6">
              ...
              >
                <CheckCircle size={18} />I have run the SQL, Check Again
              </button>
            </div>
          </div>
        </div>
                    ...
                    >
                      ...
                      <td className="p-4">
                        {u.email === "hananirfan85@gmail.com" ? (
                          <span className="px-2 py-1 bg-purple-100 text-purple-800 text-xs font-bold rounded-md uppercase tracking-wide">
                            Super Admin
                          </span>
                        ) : editingRoleFor === u.id ? (
                          <div className="flex items-center gap-2">
                            <select className="text-sm border border-gray-300 rounded-md py-1 px-2 outline-none"
                              value={newRoleStr}
              ...
              {messages.map((t) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <span className={`px-2 py-1 text-xs font-bold rounded-md uppercase tracking-wide mr-2 ${t.status === "pending" ? "bg-red-100 text-red-800" : "bg-green-100 text-green-800"}`}
                      >
                        {t.status}
                      </span>
                  ...
                  <p className="text-gray-700 mt-2 whitespace-pre-wrap rounded-lg bg-gray-100 p-4 border border-gray-200">
                    {t.message}
                  </p>

                  {t.reply ? (
                    <div className="mt-4 bg-indigo-50 border border-indigo-100 p-4 rounded-lg">
                      <strong className="text-indigo-800 text-sm block mb-1">
                        Admin Reply:
                      </strong>
                    ...
                    t.status === "pending" && (
                      <div className="mt-4">
                        <textarea className="w-full text-sm p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all resize-none"
                          rows={3}
                          ...
                          <button
                            onClick={async () => {
                              const { error } = await supabase
                                ...
                                .update({
                                  status: "replied",
                                  reply: "Marked as read/closed without reply.",
                                })
                                .eq("id", t.id);
                              if (error) {
                                alert(
