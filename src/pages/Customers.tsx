...
export default function Customers() {
  ...
  // Ledger State
  const [selectedCustomerForLedger, setSelectedCustomerForLedger] =
  ...
  const handleAddCustomer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        err.message?.includes('relation "public.customers" does not exist')
      ) {
        toast.error(
          "Table missing! Please go to Admin Panel and run the provided SQL queries in Supabase.",
          { duration: 6000 },
        );
      } else {
        toast.error(
          err.message ||
            "Failed to save customer. Make sure SQL setup is complete in Admin Panel.",
        );
      }
    }
  };
  ...
  const handleAddLedgerEntry = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .eq("id", selectedCustomerForLedger.id);
      ...
      setSelectedCustomerForLedger((prev) =>
        prev ? { ...prev, ledger_balance: newBalance } : prev,
      );
      setCustomers((prev) =>
        prev.map((c) =>
          c.id === selectedCustomerForLedger.id
            ? { ...c, ledger_balance: newBalance }
            : c,
        ),
      );
      ...
      toast.error(
        err.message ||
          "Failed to add ledger entry. Make sure SQL setup is complete.",
      );
    }
  };
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            CRM: Track loyalty, ledgers, and purchase history.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900 mt-1">
              {customers.filter((c) => c.total_purchases > 50000).length}
            </p>
          </div>
            ...
            <p className="text-2xl font-bold text-red-600 mt-1">
              {formatCurrency(
                customers.reduce(
                  (acc, c) =>
                    acc +
                    (Number(c.ledger_balance) > 0
                      ? Number(c.ledger_balance)
                      : 0),
                  0,
                ),
              )}
              ...
              {loading ? (
                ...
                filteredCustomers.map((c) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      {c.email && (
                        <div className="text-sm text-gray-600 text-sm overflow-hidden text-ellipsis max-w-[200px]">
                          {c.email}
                        </div>
                      )}
                      {c.phone && (
                    ...
                    <td className="p-4">
                      <div
                        className={`font-mono font-medium ${Number(c.ledger_balance) > 0 ? "text-red-600" : Number(c.ledger_balance) < 0 ? "text-emerald-600" : "text-gray-500"}`}
                      >
                        {Number(c.ledger_balance) > 0
                          ? "Due: "
                          : Number(c.ledger_balance) < 0
                            ...
                            : ""}
                        {formatCurrency(
                          Math.abs(Number(c.ledger_balance) || 0),
                        )}
                      </div>
                    </td>
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
              <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50 shrink-0">
                <div>
                  ...
                  <div
                    className={`mt-1 text-sm font-bold ${Number(selectedCustomerForLedger.ledger_balance) > 0 ? "text-red-600" : "text-emerald-600"}`}
                  >
                    Current Balance:{" "}
                    {Number(selectedCustomerForLedger.ledger_balance) > 0
                      ? "Due "
                      : "Advance "}
                    {formatCurrency(
                      Math.abs(
                ...
                {ledgerLoading ? (
                  ...
                  <div className="divide-y divide-gray-100">
                    {ledgerEntries.map((entry) => (
                      <div
                        key={entry.id}
                        className="p-4 flex items-center justify-between bg-white hover:bg-gray-50/50 transition"
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`p-2 rounded-full ${entry.type === "charge" ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600"}`}
                          >
                            {entry.type === "charge" ? (
                        ...
                        <div className="flex items-center gap-4">
                          <div
                            className={`font-mono font-bold text-lg ${entry.type === "charge" ? "text-red-600" : "text-emerald-600"}`}
                          >
                            {entry.type === "charge" ? "+" : "-"}
                            ...
                            onClick={async () => {
                              ...
                              try {
                                ...
                                let newBalance = Number(
                                  selectedCustomerForLedger.ledger_balance || 0,
                                );
                                ...
                                setSelectedCustomerForLedger((prev) =>
                                  prev
                                    ? { ...prev, ledger_balance: newBalance }
                                    : prev,
                                );
                                ...
                                toast.error(
                                  err.message ||
                                    "Failed to delete entry. Make sure SQL setup is complete.",
                                );
                              }
                            }}
