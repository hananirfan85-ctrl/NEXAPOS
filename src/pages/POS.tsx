...
export default function POS() {
  ...
  const cartProfit = cart.reduce(
    (sum, item) =>
      sum + (item.selling_price - item.cost_price) * item.cartQuantity,
    0,
  );
  ...
  const completeSale = async (e?: React.FormEvent) => {
    ...
    if (error) {
      ...
      alert(
        "Error processing sale. Is your Supabase RPC set up? Check the console.",
      );
  ...
  const handlePrint = () => {
    ...
    const bName =
      localStorage.getItem("nexapos_business_name") || "NEXA POS POS";
    const address =
      localStorage.getItem("nexapos_address") ||
      "Nexus Supermarket\n123 Street";
    const footer =
      localStorage.getItem("nexapos_footer") ||
      ...
      <div className="h-full flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">
            Receipt ID: {completedSale?.id?.split("-")[0].toUpperCase()}
          </p>

          <div className="border-t border-b py-4 my-6 border-dashed space-y-2">
            <div className="flex justify-between font-medium">
              <span>Amount Paid</span>
              <span>{formatCurrency(completedSale?.amount || 0)}</span>
            </div>
            <div className="flex justify-between text-gray-500 text-sm mt-1">
              <span>Cash Received</span>
              <span>{formatCurrency(completedSale?.cashReceived || 0)}</span>
            </div>
              ...
              {filteredProducts.map((product) => (
                ...
                >
                  ...
                  <div className="mt-3 flex items-end justify-between">
                    ...
                    <span
                      className={`text-xs font-medium px-2 py-1 rounded-md ${product.stock <= 5 ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-600"}`}
                    >
                      {product.stock} left
                    </span>
                  </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {cart.length > 0 && (
                <button
                  ...
                  className="text-gray-400 hover:text-red-600 transition-colors bg-white border border-gray-200 p-1 rounded-full shadow-sm ml-2"
                >
                  <Trash2 size={14} />
                </button>
              )}
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center bg-white rounded-md border border-gray-200">
                        <button
                          ...
                          className="p-1 sm:p-2 hover:bg-gray-100 text-gray-600 transition-colors"
                        >
                          <Minus size={14} />
                        </button>
                ...
                {parseFloat(cashReceived) >= cartTotal && (
                  <div className="flex justify-between items-center py-3 bg-emerald-50 px-4 rounded-lg border border-emerald-100">
                    ...
                    <span className="text-xl font-bold text-emerald-700">
                      {formatCurrency(
                        (parseFloat(cashReceived) || cartTotal) - cartTotal,
                      )}
                    </span>
                  </div>
                )}
